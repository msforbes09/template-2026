...
**Design docs live at the workspace-root `docs/superpowers/`** (`plans/` + `specs/`) — the single canonical location. Run brainstorming/planning from the workspace root so new specs land here; do not create a second `project/docs/`. See `docs/README.md` for the full documentation taxonomy (handoffs, deploy runbooks, QA plans).
...
- **Users** — website self-registration (email or SMS OTP, verify-before-create, anti-enumeration), login with OTP 2FA and trusted devices, forgot/reset password, profile (`draft` → `completed` with an edit cooldown), contact channels, password-confirmed account deletion with a recovery window. PII is encrypted (`EncryptsPii`) with keyed-HMAC blind indexes.
- **OTP** — `OtpService` with cooldown, resend and attempt lockouts; delivered by mail (`config('otp.mailables')`) or SMS (`config('otp.sms')` templates) through the queued `SendOtp` listener. SMS goes through the `App\Services\Sms\SmsSender` contract; the template binds a log-only sender — bind a real provider in `AppServiceProvider`.
...
- **Logs** — audit trail (`Audit`), auth attempts (`AuthAttempt`), outbound connections (`Connection`) on **monthly tables** (`HasMonthlyTable`, pre-created by the `*:prepare-next-table` commands), read-only admin viewers gated per type, and `log.pii-access` auditing every full-PII read. Every outbound HTTP call goes through a `ConnectionService` subclass so it is logged with secrets and PII masked.
...
- **Files** — uploads to S3 with public/private visibility, `OwnedFileRule` for IDOR-safe references, optional CloudFront signed URLs (falls back to plain URLs without a key pair), MIME/size/threat validation.
...
- External services are faked: `Tests\Fakes\FakeSmsSender` records texts, S3 uses `Storage::fake`, OpenSearch clients are mocked.
...
- **Rate limiting** — Auth-style, OTP, and enumeration-prone endpoints carry a named `throttle:` limiter, keyed per the existing pattern (IP for anonymous auth; the account key where the route is authenticated and guards a guessable secret; the normalized email/mobile for shared-NAT crowds).
...
- **Notification copy names the platform from `config('app.name')` — never hardcode it.** Every recipient-facing string (mail bodies, OTP SMS templates, subjects) interpolates the app name, and the surrounding copy stays article-free. `NotificationBrandingTest` guards this.

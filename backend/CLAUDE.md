...
- **Administrators** — auth (email + password, optional email-OTP 2FA), sliding-inactivity + absolute-lifetime token ceilings (`refresh.token`), temporary-password and expired-password gate (`password.changed`; passwords expire after `ADMIN_PASSWORD_EXPIRY_DAYS`, may be postponed `ADMIN_PASSWORD_EXPIRY_MAX_WAIVES` times via `POST password/waive-expiry`, each postponement buying `ADMIN_PASSWORD_EXPIRY_WAIVE_DAYS`, and the profile carries the effective `password_expires_at` / `is_password_expired` / `password_expiry_waives_remaining`), admin CRUD with self-escalation guards (`protect.administrator`), roles/permissions with permission groups (`PermissionEnum`, `AccessSeeder`), a sealed Super Admin role (`protect.super-admin-role`), and a virtual `developer-access` token ability granted only to `is_developer` admins (gates the feature-flag API).
...
- **Session window (both guards)** — the profile resources expose `session_inactivity_minutes` and `token_expires_at` (`Authenticates::tokenExpiresAt()`, null for a transient test token) so the frontend can warn before the sliding inactivity ceiling signs the user out; `GET profile` doubles as the keep-alive because `refresh.token` / `refresh.user.token` slide the expiry on every authenticated request.
- **Password policy (both guards)** — `HasPasswordHistory` stamps `password_changed_at` and records every hash in `password_histories` (bounded by `PASSWORD_HISTORY_LIMIT`); `NotRecentlyUsedPassword` rejects reuse on the authenticated change requests, and `resetPasswordWithOtp()` runs the same check **after** the OTP is verified (never as a request rule: a pre-auth rule would let anyone probe an address's passwords) — both as a `422` on `new_password`; `assertPasswordOldEnoughToChange()` refuses a voluntary change inside `PASSWORD_MIN_AGE_HOURS` with the `password_changed_too_recently` envelope (resets and temporary-password changes exempt). `0` disables either check.
...
- **OpenSearch (optional)** — with `SCOUT_DRIVER=opensearch` the logs, users and notifications are mirrored into OpenSearch (`SearchableLog`, `SearchableUser`), lists read from it with a MySQL fallback (`OPENSEARCH_READ_ENABLED`), and an ISM lifecycle rolls log indices over and deletes them after `OPENSEARCH_LOG_RETENTION_MONTHS` (default 3; MySQL keeps the full history). The cluster runs outside this workspace, like Redis. **Adding a document field means updating `toSearchableArray()` and `searchableMapping()` together.**
...
- **Redis and OpenSearch** are not part of the compose stack: run each from a standalone compose file on the host and point `REDIS_HOST` / `OPENSEARCH_HOST` at `host.docker.internal`. Single-node OpenSearch needs `OPENSEARCH_LOG_REPLICAS=0`.

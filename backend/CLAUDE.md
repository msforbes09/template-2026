...
- **Administrators** — auth (email + password, optional email-OTP 2FA), sliding-inactivity + absolute-lifetime token ceilings (`refresh.token`), temporary-password and expired-password gate (`password.changed`; passwords expire after `ADMIN_PASSWORD_EXPIRY_DAYS`, may be postponed `ADMIN_PASSWORD_EXPIRY_MAX_WAIVES` times via `POST password/waive-expiry`, and the profile carries `password_expires_at` / `is_password_expired` / `password_expiry_waives_remaining`), admin CRUD with self-escalation guards (`protect.administrator`), roles/permissions with permission groups (`PermissionEnum`, `AccessSeeder`), a sealed Super Admin role (`protect.super-admin-role`), and a virtual `developer-access` token ability granted only to `is_developer` admins (gates the feature-flag API).

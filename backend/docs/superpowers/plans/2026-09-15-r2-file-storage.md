...
- API contract unchanged; only the `url` host changes (public custom domain, or an R2 presigned URL valid ~3 hours for private files). Any `next/image` remote pattern or CSP `img-src` allowlist must include the new hosts.

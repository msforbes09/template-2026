...
- **Done (2026-09-16):** password history and minimum age on both guards. `password_histories` (morph `owner`, hash per set, pruned to `PASSWORD_HISTORY_LIMIT`, default 5) and `password_changed_at` on `administrators` and `users`, both maintained by `HasPasswordHistory` model events so every set path is covered; the migration backfills a history row from the current hash and stamps `password_changed_at` with the migration time (not `updated_at`, which every login bumps). `NotRecentlyUsedPassword` on the admin and user change requests, and the same check inside `resetPasswordWithOtp()` after OTP verification (`422` on `new_password`; kept out of the reset Form Request so an unauthenticated caller cannot probe passwords with a garbage code); history is written on `created` and on `updated` when the password changed (not `saved` + `wasRecentlyCreated`, which double-records on a create-then-update); `assertPasswordOldEnoughToChange()` in `changePassword()` / `updatePassword()` throws `password_changed_too_recently` (`400`, `meta.available_at`) inside `PASSWORD_MIN_AGE_HOURS` (default 24) — temporary-password changes and OTP resets exempt. Reset-to-current-password is now the `422` reuse error rather than `password_unchanged`.
- **Done (2026-09-16):** administrator password expiry with postponements. `password_expiry_waives` and `password_expiry_waived_until` on `administrators` (both reset whenever the password changes, in `Administrator::booted()`); `passwordExpiresAt()` = `password_changed_at` + `ADMIN_PASSWORD_EXPIRY_DAYS` (default 90, 0 disables) or the end of the latest postponement if later, so a waive moves the deadline `ADMIN_PASSWORD_EXPIRY_WAIVE_DAYS` (default 7) and the clock, not the client, enforces the policy; `EnsurePasswordChanged` also throws `password_expired` (`403`) when expired with no waives left; `POST administrator/password/waive-expiry` (`WaivePasswordExpiryController`, inside the authenticated group but outside `password.changed`) uses one of `ADMIN_PASSWORD_EXPIRY_MAX_WAIVES` (default 3) or answers `password_expiry_waive_unavailable` (`400`). The profile resource exposes `password_changed_at`, `password_expires_at`, `is_password_expired`, `password_expiry_waives_remaining`; the frontend gate and dashboard banner key off those.
...
- **Deferred:** `password_histories` rows outlive a soft-deleted account. Harmless (hashes only, and the account may be recovered) but they could be pruned when a user is hard-deleted; nothing hard-deletes users today.

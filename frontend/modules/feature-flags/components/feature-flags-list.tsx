...
export async function FeatureFlagsList() {
  ...
  return (
    <div className="space-y-6">
      {/* Reaching here means the developer-only fetch succeeded, which is the
          same ability the Horizon handoff requires. */}
      <section
        aria-labelledby="horizon-heading"
        className="flex items-start justify-between gap-6 rounded-xl border border-border bg-card px-5 py-5"
      >
        <div className="min-w-0">
          <h2 id="horizon-heading" className="text-sm font-medium">
            Queue dashboard
          </h2>
          <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
            Horizon shows every queue, its throughput and wait times, and lets
            you retry failed jobs. Opens in this tab with a one-minute sign-in
            link.
          </p>
        </div>
